...
eval(char *cmdline) 
{
    ...
    /*handle redirection
     * I don't konw why: If I change the position of thers code, It would trigger a fail*/
    if(tok.infile!=NULL){
	infile=Open(tok.infile,O_RDONLY,0);
    }
    if(tok.outfile!=NULL){
	    outfile=Open(tok.outfile,O_WRONLY,0);
    }	
	 ...
	 // printf("HEHEHE\n");
	   // fflush(stdout);
	    ...
	    if((pid=Fork())==0){	    	
		   ...
		   setpgid(0,0);
		   /* it's crucial to implement redirection behaviour here!!!
		    * cause' it only serves child process*/
		   Dup2(infile,0);
		   Dup2(outfile,1);
		    execve(tok.argv[0],tok.argv,environ); 
	    }
	    
      /*temporarily blcoking signals*/
	    ...
	    if(bg){
		    ...
		    Bg2Fg(pid);/*change background job to foreground job*/
	    }
...
/*changing a process to frontground by waiting for its reaping*/
void Bg2Fg(int pid){

//	   printf("AAAAAAAAAAAA%d\n",pid);
//	   fflush(stdout);
	struct job_t * job;
	sigset_t mask_one,mask_all,pre_all;
	sigemptyset(&mask_all);
	sigaddset(&mask_all,SIGCHLD);
	sigaddset(&mask_all,SIGINT);
	sigaddset(&mask_all,SIGTSTP);
	...
	sigdelset(&mask_one,SIGINT);
	sigdelset(&mask_one,SIGTSTP);
